...
double Random :: Exp(double lambda) {
   ...
   double y=Rannyu();
   double x=-log(1-y)/lambda;
   return x;
}

double Random :: Lorentz(double gamma, double mean) {
   // This function generates a random number from an Lorantzian distribution with given lambda
   double y=Rannyu();
   double x=mean+gamma*tan(M_PI*(y-1/2));
   return x;
}
...
int Random::RandInt(int min, int max) {
    if (max <= min) return min;              // gestione caso "strano"
    int range = max - min;                   // range > 0
    return min + (int)std::floor(Rannyu() * range);  // [min, max)
    // equivalente anche: return min + (int)(Rannyu() * range);
}
